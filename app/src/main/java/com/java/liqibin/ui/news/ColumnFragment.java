...
public class ColumnFragment extends Fragment {
    ...
    public static ColumnFragment newInstance(String columnName) {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        columnName = new String[]{getArguments().getString("columnName")};
        ...
        refreshLayout.setOnRefreshListener((layout) -> {
            if (CheckNetworkState.isNetwordConnected(activity)) {
                ...
                new RefreshTask(activity, newsList, showEmpty, (SmartRefreshLayout) layout)
                        .executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR, new NewsQuery().setCategories(columnName[0]));
            } else {
                offline = true;
            ...
            if (CheckNetworkState.isNetwordConnected(activity)) {
                if (offline) {
                    ...
                    new LoadMoreTask(activity, newsList, showEmpty, (SmartRefreshLayout) layout)
                            .executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR, new NewsQuery().setCategories(columnName[0]).setPage(page));
                }
            } else {
                if (!offline) {
